...
const Player = () => {
    ...
    const { seekBar, track, time, seekBg, playStatus, play, pause, next, previous, seekSong, volumeBar, setVolume, audioRef, toggleMute } = useContext(PlayerContext)
        ...
        <div className='h-[10%] bg-black flex justify-between items-center text-white px-4'>
            ...
            <div className='hidden lg:flex items-center gap-4 opacity-75'>
                ...
                {audioRef.current && audioRef.current.volume > 0
                    ...
                    : <img onClick={toggleMute} className='w-4 cursor-pointer' src={assets.mute_icon} alt="" />
                }
                <div ref={volumeBar} onClick={setVolume} className='w-20 bg-slate-50 h-1 rounded cursor-pointer'>
                    <hr className='h-1 border-none bg-green-800 rounded-full' style={{ width: `${audioRef.current ? audioRef.current.volume * 100 : 50}%` }} />
                </div>
